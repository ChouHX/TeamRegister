...
AUTO_PARAMS = {
    "proxy": "",                        # 留空=沿用 config.json / 环境变量代理
...
def _cpa_request_with_fallback(method: str, url: str, *, cfg: dict, **kwargs):
    try:
    ...
    last_exception = None
    proxy_candidates = []
    for item in _resolve_cpa_proxy_candidates(cfg):
    ...
    for idx, proxy in enumerate(proxy_candidates):
        ...
        try:
            ...
            if idx < len(proxy_candidates) - 1:
                ...
                print(f"[检测] CPA 请求经{mode}失败，尝试下一种方式: {e}")
                continue
            raise
        finally:
            if session:
...
def count_valid_accounts_by_probe(cfg: dict) -> int:
    ...
    # ---- 1. 拉取 auth-files 列表 ----
    try:
        resp = _cpa_request_with_fallback("GET", list_url, cfg=cfg, headers=headers, timeout=15)
